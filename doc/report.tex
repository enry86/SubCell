...
respect to the cell and its structure. Each protein is composed by a sequence
of amino-acids, that is a string of letters which represent its one-dimensional
structure. The target is to localize the protein considering the
cell environment, in particular, define if it belongs into one of the following
possibilities: the Cytoplasm, the Nucleus, the Mitochondria or if it's Secreted
by the cell.

The classifier works on a dataset formed by animal proteins which belong to 
different Natural Kingdoms. Changing the Kingdom changes also the position
of the protein in the cell environment. By the scope of the classification,
it is no important what Kingdom the cell belongs to.
...
possible training parameters which affects the single SVM.
